...
func (f BundleFinalizer) Finalize(ctx context.Context, bid, out string) (domain.BatchDescriptor, error) {
	...
	for _, p := range ps {
		if p.Status != domain.PackReady || p.SHA256 == "" {
			return d, fmt.Errorf("pack %s not ready", p.ID)
		}
	}
	if f.OpenSQLite == nil {
		return d, fmt.Errorf("sqlite opener required")
	}
	...
	if e = pack.InitManifest(ctx, db); e != nil {
		db.Close()
		return d, e
	}
	...
	defer tx.Rollback()
	if _, e = tx.ExecContext(ctx, `INSERT INTO manifest_meta(key,value)VALUES('schema_version','1'),('source_id',?),('epoch_id',?),('batch_id',?)`, b.SourceID, b.EpochID, b.ID); e != nil {
		return d, e
	}
	...
	for _, u := range units {
		if _, e = tx.ExecContext(ctx, `INSERT INTO publish_units(id,epoch_id,source_id,unit_key,required_count,metadata_path)VALUES(?,?,?,?,?,?)`, u.ID, u.EpochID, u.SourceID, u.Key, u.Required, u.MetadataPath); e != nil {
			return d, e
		}
	}
	if e = f.Plan.WalkBatchManifestEntries(ctx, b.ID, func(m domain.ManifestEntry) error {
		a := m.Artifact
		if a.SHA256 == "" {
			return fmt.Errorf("artifact %s not finalized", a.ID)
		}
	...
	for _, p := range ps {
		d.Packs = append(d.Packs, domain.PackSpec{ID: p.ID, Sequence: p.Sequence, File: filepath.ToSlash(filepath.Join("packs", fmt.Sprintf("%06d.agp", p.Sequence))), Size: p.Size, SHA256: p.SHA256, EntryCount: p.EntryCount})
	}
	body, e := json.MarshalIndent(d, "", "  ")
	if e != nil {
		return d, e
	}
	if e = os.WriteFile(filepath.Join(out, "batch.json"), body, 0644); e != nil {
		return d, e
	}
	b.Status = domain.BatchReady
	if e = f.Exec.UpdateBatch(ctx, b); e != nil {
		return d, e
	}
	return d, nil
}
